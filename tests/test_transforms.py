...
def test_frequency_energy_round_trip() -> None:
    ...
    freq_phz = transforms.convert_axis_from_nm(nm, "frequency_phz")
    back_nm_phz = transforms.convert_axis_to_nm(freq_phz, "frequency_phz")
    assert np.allclose(nm, back_nm_phz)
    ...
    energy_kev = transforms.convert_axis_from_nm(nm, "energy_kev")
    back_nm_energy_kev = transforms.convert_axis_to_nm(energy_kev, "energy_kev")
    assert np.allclose(nm, back_nm_energy_kev)
    energy_mev = transforms.convert_axis_from_nm(nm, "energy_mev")
    back_nm_energy_mev = transforms.convert_axis_to_nm(energy_mev, "energy_mev")
    assert np.allclose(nm, back_nm_energy_mev)
